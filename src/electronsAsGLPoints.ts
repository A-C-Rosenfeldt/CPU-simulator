...
export function particle(canvasId: string, strokes: number[][], extent: number[]) {
   if (! Array.isArray(strokes)) console.warn("strokes need to be an array")
   if (strokes.length < 1) console.warn("why send 0 strokes?")
   if (! Array.isArray(strokes[0]) ) console.warn("storke needs to be array of array")
   if ( strokes[0].length ==0) console.warn("why stroke of zero lenght?")
