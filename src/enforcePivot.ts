...
export class JoinOperatorIterator {
    ...
    constructor(...keyValues: number[][]);
    constructor(initi: boolean, ...keyValues: number[][]);
...
export interface SeamlessValues {
...
export class AllSeamless implements SeamlessValues {
    private _Value_next: number[][] = []
    ...
    public Value_push(value: number[]) {
                ...
                if (whatIf) {
                ...
                } else {
                    {
                        ...
                        { // flush .. sure this gap will have length>0 .. seems I need 3 {gap,pos}
                            ...
                            if (this._Value_next.length > (this._keyValue_next.length >> 1)) { throw "I could not belive it, but log claims 1: " + this._Value_next.length + " > " + this._keyValue_next.length + " >>1  " }
    ...
    flush() {
        if (this.filled[1] /* condition derived from log while debugging. Maybe test? */) { // to keep even numbeer of open and close .. looks better in log .. also even worse than a seam (aesthetically) .  todo test for this
            ...
            if (this._Value_next.length > (this._keyValue_next.length >> 1)) { throw "I could not belive it, but log claims 2: " + this._Value_next.length + " > " + this._keyValue_next.length + " >>1  " }
        } else {
...
export class Row {
    ...
    private static StaticSub(jop: JopWithRefToValue, factor: number, nukeCol: number) {
    ...
    public shiftedOverlay(heightKeyValue: number, swapBordersOriginalConcatDelayed: number[], spans_new_Stream: SeamlessValues[] /* out parameter used for a mock in a unit test, sorry */, dropColumn = false, print=false) {
        ...
        while ((pos = jop.next()) < jop.behindKeyValue) {if (pos>= heightKeyValue) break }/* we only care for the overlap ..  = flush does this. Also in jop all source cursors advance in lock step */ /* jop.behind */ /* could be replaced by < Matrix.width */
...
export class Tridiagonal implements Matrix {
    ...
    constructor(length: number | Row[]) {
        if (typeof length === "number") { this.row = new Array(length) }
        else {
            this.row = length;
        }
    ...
    PrintGl(showSpans = false): SimpleImage {
