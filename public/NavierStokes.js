...
class Liquid_Lattice {
    constructor() {
        this.mu = 1; // viscosity needs to be tuned for a nice look 
        this.t = 0;
    }
}
class StateEquation {
    pressuer(particle_count, temperature) {
        return particle_count * temperature; // todo: Space -self - charge? Density ^2 ?
    }
}
//class Vector{}
...
class Buffer {
    constructor() {
        //clear
        //swap
        this.grid = new Liquid_Lattice[2]; // lattice (components) and buffer (localization .. see the slightly different fields.IterateOverAllCells)
        this.NS = new Navier_Stokes();
    }
    propagate() {
        ...
        for (let i = 0; i < this.grid[1].length - 1; i++) {
            ...
            for (let k = 0; k < str.length - 1; k++) {
                ...
                if (i > 0 && k > 0) { // i,k count backwards? void function with return or switch with break? JS like all the C lika languages has continue
                    //force
                    let force = stress[i + 1][k] - stress[i][k] + stress[i][k + 1] - stress[i][k]; // I need stress on the same locations as the strain ( main component )
        ...
        //NS
        Strain(window.beforeEach(velocity), this.grid[2].forEach((strain) => {
    ...
    self_consistent_naive_direction() {
        let velocity = [[[]]]; // rough -> name field vector -> fine  . Liquid_Lattice{x,y} but only velocity . component c
...
let strain = [new Vector(0, 0), new Vector(0, 0)]; // like clear?
for (var di = 0; di < 2; di++)
    for (var dk = 0; dk < 2; dk++) { // from FieldToDiagonalShapeToSparseMatrix
        const si = i - 1 + (di + dk); //  s=source=pull   // 00 01 10 11  monotonous increase
...
let r;
let derivatives;
let vector_of_derivatives = [[r[0], v[0], a[0]], [r[1], v[1], a[1]]];
...
Trace(r_parts, Array < Array < number >> , v, Array < number > , a, Array(), number, {
...
let cd = coeff.map(derivative => {
    let d = derivative.Map(dimension => dimension.reduce((s, part) => s + part) * factor);
    factor = 1;
});
...
[Array < number > , /*=*/ Tridiagonal /* x U*/];
{
}
Print();
ImageData;
{ //ToPicture   print=text vs picture?
...
function laplace(v) {
    return +v[y + 1][0][1] - v[y - 1][0][1] +
        v[0][x + 1][0] - v[0][x - 1][0];
}
function timeStep() {
    var m;
    m + nabla_cdot(v) * m;
    -nabla_cdot(p) + delta(add(v[y][x]));
}
class Collision {
}
export {};
