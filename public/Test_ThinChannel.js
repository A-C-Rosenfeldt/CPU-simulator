...
function animate() {
    ...
    for (let sweep = 0; sweep < sweep_resolution + overflow; sweep++) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (i == 1)
                voltages = [(si % 7 < 3 ? 1 : 0), (si1 % 7 < 3 ? 1 : 0)]; // >>(1+i) & 1,si1>>(1+i) & 1]   // I think that in C this is actually undefined behaviour :-(
            gates[1 - i].Voltage = 1.1 * (voltages[0] * (1 - f) + f * voltages[1]);
        }
    }
    let pixel2 = new Uint8Array(buffer);
    let si = { pixel: pixel2, width: cm, height: sweep_resolution + overflow };
