...
describe('trim row (Accessing my code)', () => {
    it('shOuld reTurn [4]', () => {
        //const scalar = new Tridiagonal(1);
        const r = enforcePivot_1.Row.Single(0, 4); // new 0,[[],[4],[]]) // Faktor 20
        (0, chai_1.expect)(r.data[0].length).to.equal(1);
...
describe('Matrix Accessing my code', () => {
    it('shOuld reTurn 4', () => {
        const scalar = new enforcePivot_1.Tridiagonal(1);
        scalar.row[0] = enforcePivot_1.Row.Single(0, 4); // new 0,[[],[4],[]]) // Faktor 20
...
describe('Testing unequi-join', () => {
    it('shOuld return 0', () => {
        const jop = new enforcePivot_1.JoinOperatorIterator([0, 3, 4, 9], [1, 4, 6, 8]);
        let result = jop.next();
        ...
        (0, chai_1.expect)(result).to.equal(4);
        result = jop.next();
        (0, chai_1.expect)(result).to.equal(6);
        result = jop.next();
        (0, chai_1.expect)(result).to.equal(8);
        result = jop.next();
        (0, chai_1.expect)(result).to.equal(9);
        result = jop.next();
        (0, chai_1.expect)(result).to.equal(jop.behind);
        (0, chai_1.expect)(jop.i.every(v => v.filled === false)).to.true;
    });
});
// before(async () => {  
...
describe('Testing Seamless', () => {
    let sea = new enforcePivot_1.AllSeamless();
    beforeEach(function () {
    ...
    it('empty, false, flush', () => {
        ...
        (0, chai_1.expect)(sea.filled[1]).to.equal(false); // not yet trigger data concat !
        (0, chai_1.expect)(sea.filled[0]).to.equal(false); // parameter
        /*
 ...
 .. now, let's test
      */
        sea.flush();
        (0, chai_1.expect)(sea.start_next.length).to.equal(0);
    });
    it('extend, true, flush', () => {
        const span = new enforcePivot_1.Span(3, 2);
        ...
        (0, chai_1.expect)(sea.pos_input).to.equal(2); //0)  
        ...
        (0, chai_1.expect)(sea.filled[1]).to.equal(false);
        (0, chai_1.expect)(sea.filled[0]).to.equal(true);
        sea.flush();
        (0, chai_1.expect)(sea.start_next.length).to.equal(0);
        return;
        (0, chai_1.expect)(sea.start_next.length).to.equal(1); //  Note pos[1] . jop again detects filled state => no real border, just a seam. Do not note!
        (0, chai_1.expect)(sea.start_next[0]).to.equal(2);
        (0, chai_1.expect)(sea.data_next.length).to.equal(0); // Make sure concatting happens on true->false transition.  values still reside in concatter    
        (0, chai_1.expect)(sea.pos_input[0]).to.equal(5); // { // eat zero length  ( Row constructor does this too, but it is only one line ). No code outside this block!
        (0, chai_1.expect)(sea.pos_input[1]).to.equal(2);
        // jop is at the beginning. Nothing to write yet    
        (0, chai_1.expect)(sea.start_next.length).to.equal(1, "nothing in output yet"); //2020-12-28  start_nest is undefined
        // but lets test with "accidental input" sea.flush()
        const span1 = new enforcePivot_1.Span(1, 3);
        ...
        (0, chai_1.expect)(sea.start_next.length).to.equal(1); // still filling
        (0, chai_1.expect)(sea.data_next.length).to.equal(0); // still filling
        sea.flush();
        (0, chai_1.expect)(sea.start_next.length).to.equal(2); // this "start" is really a "stop"
        (0, chai_1.expect)(sea.data_next.length).to.equal(1); // data  from start to stop
        // ancient: expect(sea.data_next[0].length).to.equal(3);  // one fused span from 2..5  5-2=3
        let result = sea.start_next; //hello();
        (0, chai_1.expect)(result[0]).to.equal(2);
        (0, chai_1.expect)(result[1]).to.equal(5);
        const resultd = sea.data_next[0]; //hello();
        (0, chai_1.expect)(resultd[0]).to.equal(2);
        (0, chai_1.expect)(resultd[1]).to.equal(3);
        (0, chai_1.expect)(resultd[2]).to.equal(4);
    });
});
// sub now uses quite complicated helper classes. Even private classes should be tested. I mean, with fields within classes it is probably difficult to not destroy the tests, but with classes?
describe('Testing unequi-join integrate Seamless', () => {
    it('shOuld return 0', () => {
        ...
        const span1 = new enforcePivot_1.Span(3, 0);
        const span2 = new enforcePivot_1.Span(3, 1);
        const jop = new enforcePivot_1.JoinOperatorIterator([0, 3, 4, 9], [1, 4, 6, 8]);
        span1.extends = [12, 13, 14];
        span2.extends = [22, 23, 24];
        const sea = new enforcePivot_1.AllSeamless();
        let result = jop.next();
        (0, chai_1.expect)(result).to.equal(0);
        const these = jop.i.every(ii => {
            //  const ii=jop.i[1][j]
        ...
        (0, chai_1.expect)(jop.i[0].filled).to.true;
        (0, chai_1.expect)(jop.i[1].filled).to.false;
        sea.removeSeams([span1], result, true);
        (0, chai_1.expect)(sea.pos_input).to.equal(0); // this is a little bit lame. Todo: better test data. Push this edge case towards the end.
        // Inf fact: I do not know .. ah I know, both I should guard the next edge
        // expect(jop.i[0][0]).to.equal(0);
        // expect(jop.i[0][0]).to.equal(1);
        result = jop.next();
        ...
        (0, chai_1.expect)(sea.pos_input).to.equal(1);
