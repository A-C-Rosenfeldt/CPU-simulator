...
import { Tridiagonal, Row, JoinOperatorIterator, AllSeamless, KeyValueValue } from '../src/enforcePivot';
...
describe('trim row (Accessing my code)', () => {
  it('shOuld reTurn [4]', () => {
      ...
      expect(r.Value[0].length).to.equal(1);
      const result = r.Value[0][0]; //hello();
      expect(result).to.equal(4);
  });
});
...
describe('key value pairs like classical sparse matrix', () => {
  it('should connect dangling bonds', () => {
      ...
      expect(r.KeyValue.length).to.equal(2);
      expect(r.KeyValue[0]).to.equal(3);
      expect(r.KeyValue[1]).to.equal(5);
      expect(r.Value[0].length).to.equal(2);
      const result = r.Value[0][0]; //hello();
      expect(result).to.equal(6);
      expect(r.Value[0][1]).to.equal(7);
  });
  ...
  it('should trim -- and there is a gap', () => {
    ...
    // rest like above
    expect(r.KeyValue.length).to.equal(4);
    expect(r.KeyValue[0]).to.equal(3);
    expect(r.KeyValue[1]).to.equal(4);
    expect(r.KeyValue[2]).to.equal(5);
    expect(r.KeyValue[3]).to.equal(6);
    expect(r.Value[0].length).to.equal(1);
    const result = r.Value[0][0]; //hello();
    expect(result).to.equal(6);
    expect(r.Value[1][0]).to.equal(7);
  });
  it('should trim inside the gap', () => {
    ...
    // rest like above
    expect(r.KeyValue.length).to.equal(4);
    expect(r.KeyValue[0]).to.equal(2);
    expect(r.KeyValue[1]).to.equal(3);
    expect(r.KeyValue[2]).to.equal(6);
    expect(r.KeyValue[3]).to.equal(7);
    expect(r.Value[0].length).to.equal(1);
    const result = r.Value[0][0]; //hello();
    expect(result).to.equal(6);
    expect(r.Value[1][0]).to.equal(7);
...
describe('Testing Seamless', () => {
  ...
  it('empty, false, flush', () => {
    ...
    // transparent box access too risky for usage //expect(sea.KeyValue_next.length).to.equal(0); // we switch from empty ( Matrix out of bounds has 0) to filled
...
.. now, let's test
     */
    sea.flush()
    expect(sea.KeyValue_next.length).to.equal(0)
  })

  
  it('extend, true, flush', () => {
 
    const span=new KeyValueValue<number>(3,2)
    span.Value=[2,3,4]  // i >> 1  can give the string one cycle after getting the start value. After all we need to wait for end
...
// what does thar mean?    //expect(sea.pos_input[0]).to.equal(2) // { // eat zero length  ( Row constructor does this too, but it is only one line ). No code outside this block!
    expect(sea.keyValue_input).to.equal(2) //0)  
    ...
    expect(sea.KeyValue_next.length).to.equal(0) 
    return
    expect(sea.KeyValue_next.length).to.equal(1); //  Note pos[1] . jop again detects filled state => no real border, just a seam. Do not note!
    expect(sea.KeyValue_next[0]).to.equal(2);
    ...
    expect(sea.keyValue_input[0]).to.equal(5) // { // eat zero length  ( Row constructor does this too, but it is only one line ). No code outside this block!
    expect(sea.keyValue_input[1]).to.equal(2)  

    // jop is at the beginning. Nothing to write yet    
    expect(sea.KeyValue_next.length).to.equal(1,"nothing in output yet"); //2020-12-28  start_nest is undefined

    // but lets test with "accidental input" sea.flush()
    
    const span1=new KeyValueValue<number>(1,3)
    span1.Value=[1 /* len < 1 is ignored */]
    ...
    expect(sea.KeyValue_next.length).to.equal(1); // still filling
    expect(sea.data_next.length).to.equal(0); // still filling
    sea.flush();
    expect(sea.KeyValue_next.length).to.equal(2); // this "start" is really a "stop"
    ...
    let result = sea.KeyValue_next //hello();
    ...
    describe('Testing unique-join integrate Seamless', () => {
      ...
      it('shOuld return 0', () => {
        ...
        const span1=new KeyValueValue<number>(3,0)                
        const span2=new KeyValueValue<number>(3,1)

        const jop=new JoinOperatorIterator([0,3,4,9],[1,4,6,8])
        span1.Value=[12,13,14]
        span2.Value=[22,23,24]         
        ...
        expect(sea.keyValue_input).to.equal(1) 
