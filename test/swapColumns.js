...
class SeamlessMock {
    ...
    removeSeams(fillValues, pos, filled, operator, nukeCol, whatIf) {
...
describe('Swap', () => {
    it('jop should work with three operands jop and cutter at 0 and row.length', () => {
        let jop = new enforcePivot_1.JoinOperatorIterator([0, 1], [3, 4], [0, 0, 3, 4, 6, 6]);
        // find minimum start value of the three
        let pos = jop.next();
        (0, chai_1.expect)(pos).to.equal(0);
        // white box
        {
            let i = 0;
            (0, chai_1.expect)(jop.i[i++].from).to.equal(1);
            (0, chai_1.expect)(jop.i[i++].from).to.equal(0); // shifted values start later
            (0, chai_1.expect)(jop.i[i++].from).to.equal(1);
        }
        pos = jop.next();
        (0, chai_1.expect)(pos).to.equal(0);
        // zero length cutter  needed because I do not want to check for each Row. Swap is one loop outside => easy debug
        {
            ...
            (0, chai_1.expect)(jop.i[i++].from).to.equal(1);
            (0, chai_1.expect)(jop.i[i++].from).to.equal(0); // shifted values start later
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2);
        }
        ...
        pos = jop.next();
        (0, chai_1.expect)(pos).to.equal(1);
        // zero length cutter  needed because I do not want to check for each Row. Swap is one loop outside => easy debug
        {
            let i = 0;
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2);
        ...
        // }
        pos = jop.next();
        (0, chai_1.expect)(pos).to.equal(3);
        // zero length cutter  needed because I do not want to check for each Row. Swap is one loop outside => easy debug
        {
            let i = 0;
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2);
            (0, chai_1.expect)(jop.i[i++].from).to.equal(1); // shifted values start later
            (0, chai_1.expect)(jop.i[i++].from).to.equal(3);
        }
        pos = jop.next();
        (0, chai_1.expect)(pos).to.equal(4);
        // zero length cutter  needed because I do not want to check for each Row. Swap is one loop outside => easy debug
        {
            let i = 0;
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2);
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2); // shifted values start later
            (0, chai_1.expect)(jop.i[i++].from).to.equal(4);
        }
        pos = jop.next();
        (0, chai_1.expect)(pos).to.equal(6);
        // zero length cutter  needed because I do not want to check for each Row. Swap is one loop outside => easy debug
        {
            let i = 0;
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2);
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2); // shifted values start later
            (0, chai_1.expect)(jop.i[i++].from).to.equal(5);
        }
        pos = jop.next();
        (0, chai_1.expect)(pos).to.equal(6);
        // zero length cutter  needed because I do not want to check for each Row. Swap is one loop outside => easy debug
        {
            ...
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2);
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2); // shifted values start later
            (0, chai_1.expect)(jop.i[i++].from).to.equal(6);
        }
        pos = jop.next();
        (0, chai_1.expect)(pos).to.equal(7);
        // zero length cutter  needed because I do not want to check for each Row. Swap is one loop outside => easy debug
        {
            let i = 0;
            (0, chai_1.expect)(jop.i[i++].from).to.equal(2);
    ...
    it('jop + swap', () => {
        ...
        let size = 6;
        const row = enforcePivot_1.Row.Single(0, 5); //0,[[],[5],[]])
        const swapHalf = [0, 1];
        ...
        (0, chai_1.expect)(spans_new_Stream[1].start_next.length).to.equal(2);
        (0, chai_1.expect)(spans_new_Stream[0].start_next.length).to.equal(2);
        let j = 0;
        {
            let i = 0;
            (0, chai_1.expect)(spans_new_Stream[j].start_next[i]).to.equal(3);
            (0, chai_1.expect)(spans_new_Stream[j].otherLog[i++][0]).to.equal(0); // is 1 .. like no swap happend :-(
            (0, chai_1.expect)(spans_new_Stream[j].start_next[i]).to.equal(4);
            (0, chai_1.expect)(spans_new_Stream[j].otherLog[i++][0]).to.equal(0);
        }
        j++;
        {
            let i = 0;
            (0, chai_1.expect)(spans_new_Stream[j].start_next[i]).to.equal(3);
            (0, chai_1.expect)(spans_new_Stream[j].otherLog[i++][0]).to.equal(1);
            (0, chai_1.expect)(spans_new_Stream[j].start_next[i]).to.equal(4);
            (0, chai_1.expect)(spans_new_Stream[j].otherLog[i++][0]).to.equal(0);
        }
    ...
    it('jop + swap + seamless + tridiagonal', () => {
        ...
        // to left
        (0, chai_1.expect)(unit.getAt(3, 0)).to.equal(0);
        (0, chai_1.expect)(unit.getAt(3, 3)).to.equal(5);
        // to right
        (0, chai_1.expect)(unit.getAt(0, 0)).to.equal(5);
        (0, chai_1.expect)(unit.getAt(0, 3)).to.equal(0);
        unit.swapColumns([0, 1]);
        (0, chai_1.expect)(unit.row[0].starts.length).to.equal(2);
        (0, chai_1.expect)(unit.row[0].starts[0]).to.equal(3);
        (0, chai_1.expect)(unit.row[0].starts[1]).to.equal(4);
        (0, chai_1.expect)(unit.row[0].data.length).to.equal(1);
        (0, chai_1.expect)(unit.row[0].data[0][0]).to.equal(5);
        // to left
        (0, chai_1.expect)(unit.getAt(3, 0)).to.equal(5);
        (0, chai_1.expect)(unit.getAt(3, 3)).to.equal(0);
        // to right
        (0, chai_1.expect)(unit.getAt(0, 0)).to.equal(0);
        (0, chai_1.expect)(unit.getAt(0, 3)).to.equal(5);
        (0, chai_1.expect)(unit.getAt(1, 1)).to.equal(5);
        (0, chai_1.expect)(unit.getAt(1, 4)).to.equal(0);
        unit.swapColumns([1, 3]);
        (0, chai_1.expect)(unit.getAt(1, 1)).to.equal(0);
        (0, chai_1.expect)(unit.getAt(1, 4)).to.equal(5);
    });
});
